...
const repositoryView_1 = __webpack_require__(2);
const travisStatusBarItem_1 = __webpack_require__(59);
const ActiveRepositorySingleton_1 = __webpack_require__(9);
const axios = __webpack_require__(14);
const apiTemplates_1 = __webpack_require__(8);
function activate(context) {
    // initialise singleton
    let singletonInstance;
    if (vscode_1.workspace.rootPath) {
        const singleton = ActiveRepositorySingleton_1.ActiveRepositorySingleton.createInstance(context, vscode_1.workspace.rootPath);
        singletonInstance = ActiveRepositorySingleton_1.ActiveRepositorySingleton.getInstance();
    }
    const repoView = new repositoryView_1.RepositoryView(context);
    const tree = repoView.initialise();
    const travisStatusBarInstance = new travisStatusBarItem_1.TravisStatusBar(context);
    const disposable = vscode_1.commands.registerCommand('extension.theTravisClient', () => tree);
    // Configuration change trigger events
    const onChangeConfigurationDisposable = vscode_1.workspace.onDidChangeConfiguration((event) => {
        ...
        if (onChangeBranches || onChangePro || onChangeOwner || onChangeInterval) {
            vscode_1.commands.executeCommand('theTravisClient.refresh');
        }
    });
    const setProToken = vscode_1.commands.registerCommand('theTravisClient.setProToken', () => singletonInstance.setAuthToken('enterprise'));
    const setToken = vscode_1.commands.registerCommand('theTravisClient.setToken', () => singletonInstance.setAuthToken('community'));
    ...
    const refresh = vscode_1.commands.registerCommand('theTravisClient.refresh', () => {
        const repoView = new repositoryView_1.RepositoryView(context);
        repoView.initialise();
        travisStatusBarInstance.updateStatusBar(true);
    });
    const restart = vscode_1.commands.registerCommand('theTravisClient.restart', (data) => __awaiter(this, void 0, void 0, function* () {
        if (data && data.buildId) {
            try {
                const response = yield axios.post(apiTemplates_1.restartBuildTemplate({
                    base: singletonInstance.base(),
                    buildId: data.buildId
                }), {}, { headers: singletonInstance.headers() });
                if (response.status === 202) {
                    vscode_1.window.showInformationMessage(`Sucessfully ${data.buildId} build is restarted`);
                    vscode_1.commands.executeCommand('theTravisClient.refresh');
                }
            }
            catch (e) {
    ...
    const cancel = vscode_1.commands.registerCommand('theTravisClient.cancel', (data) => __awaiter(this, void 0, void 0, function* () {
        if (data && data.buildId) {
            try {
                const response = yield axios.post(apiTemplates_1.cancelBuildTemplate({
                    base: singletonInstance.base(),
                    buildId: data.buildId
                }), {}, { headers: singletonInstance.headers() });
                if (response.status === 202) {
                    vscode_1.window.showInformationMessage(`Sucessfully ${data.buildId} build is canceled`);
                    vscode_1.commands.executeCommand('theTravisClient.refresh');
                }
            }
            catch (e) {
...
const showErrorMessage = (status) => {
    switch (status) {
        case 403:
            vscode_1.window.showErrorMessage('You do not have permission to restart build!');
            break;
        case 404:
            vscode_1.window.showErrorMessage('Build is not found!');
            break;
        default:
...
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_1 = __webpack_require__(1);
const repositoryNodeProvider_1 = __webpack_require__(3);
class RepositoryView {
    constructor(context) {
        this.context = context;
        this.repoNodeProvider = new repositoryNodeProvider_1.RepoNodeProvider(this.context);
    }
    initialise() {
        this._tree = vscode_1.window.registerTreeDataProvider('repositories', this.repoNodeProvider);
        return this._tree;
    }
}
exports.RepositoryView = RepositoryView;
...
const repositories_1 = __webpack_require__(7);
const ActiveRepositorySingleton_1 = __webpack_require__(9);
class RepoNodeProvider {
    constructor(context) {
        this.context = context;
        this._onDidChangeTreeData = new vscode_1.EventEmitter();
        this.onDidChangeTreeData = this._onDidChangeTreeData.event;
        this.repoInstance = new repositories_1.Repositories(this.context);
        if (vscode_1.workspace.rootPath) {
            this.singleton = ActiveRepositorySingleton_1.ActiveRepositorySingleton.getInstance();
        }
    }
    refresh() {
    ...
    getPreparedData(element) {
        return __awaiter(this, void 0, void 0, function* () {
            if (element) {
                if (!element.prevData) {
                    return [];
                }
                return _.map(element.prevData.data, (d, k) => {
                    if (_.isArray(element.prevData.data)) {
                        return new Dependency(this.getTimeInfo(d), d.state, d.id, d, vscode_1.TreeItemCollapsibleState.None);
                    }
                    return new Dependency(k, 'branch', k, { data: d }, this.singleton.branch() === k ? vscode_1.TreeItemCollapsibleState.Expanded : vscode_1.TreeItemCollapsibleState.Collapsed);
                });
            }
            try {
                const data = yield this.repoInstance.loadData();
                const preparedData = _.chain(data)
                    ...
                    state: 'repository',
                    id: '',
                    data: Object.assign({}, _.groupBy(v, 'branch.name'))
                }))
                    .value();
                return _.map(preparedData, eachData => {
                    return new Dependency(eachData.name, eachData.state, eachData.name, eachData, this.singleton.repository() === eachData.name ? vscode_1.TreeItemCollapsibleState.Expanded : vscode_1.TreeItemCollapsibleState.Collapsed);
                });
            }
            catch (e) {
                if (e.response.status === 403) {
                    vscode_1.window.showErrorMessage('Authentication error: invalid token');
                    return Promise.resolve([new Dependency('Api token error!', 'errored', 'api token', {}, vscode_1.TreeItemCollapsibleState.None, 'no-dependency')]);
                }
                else if (e.response.status === 404) {
                    vscode_1.window.showErrorMessage('Owner/Repository not found!');
                    return Promise.resolve([new Dependency('Owner/Repository not found!', 'errored', 'owner/repository not found', {}, vscode_1.TreeItemCollapsibleState.None, 'no-dependency')]);
                }
                vscode_1.window.showErrorMessage(e.message);
                return Promise.resolve([new Dependency(e.message, 'errored', e.response.status, {}, vscode_1.TreeItemCollapsibleState.None, 'no-dependency')]);
            }
        });
    }
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            if (vscode_1.workspace.rootPath && this.singleton && this.singleton.isTravisProject()) {
                const token = this.singleton.token();
                if (token) {
                    const data = yield this.getPreparedData(element);
                    return data;
                }
                vscode_1.window.showErrorMessage('You have not added token, please add to get repositories.');
                this.singleton.setAuthToken();
                return [new Dependency('Add api-token', 'info', 'api', {}, vscode_1.TreeItemCollapsibleState.None, 'no-dependency')];
            }
            vscode_1.window.showErrorMessage('This is not a travis project!');
            return [new Dependency('Not a travis project', 'errored', 'not a travis project', {}, vscode_1.TreeItemCollapsibleState.None, 'no-dependency')];
        });
    }
}
exports.RepoNodeProvider = RepoNodeProvider;
class Dependency extends vscode_1.TreeItem {
    constructor(label, state, buildId, prevData, collapsibleState, ctxValue, command) {
        ...
        this.contextValue = this.ctxValue ? this.ctxValue : _.includes(['repository', 'branch', 'info', 'loading'], this.state) ? 'no-dependency' : 'dependency';
    }
    get tooltip() {
        if (this.state) {
...
class Repositories {
    ...
    getRepositories() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield axios.get(apiTemplates_1.repositoryURLTemplate({
    ...
    getBranches() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const branchesPromise = _.map(repos, (rep) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        const response = yield axios.get(apiTemplates_1.branchesURLTemplate({ base: this.singleton.base(), repoId: rep.id }), { headers: this.headers });
    ...
    getBuilds() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const branches = yield this.getBranches();
                const actualBranches = _.filter(branches, b => !_.isEmpty(b));
                const filteredBranches = [];
                const showableBranches = vscode_1.workspace.getConfiguration('travisClient').get('branches', []);
                ...
                const buildsPromise = _.map(filteredBranches, (br) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        const response = yield axios.get(apiTemplates_1.buildsURLTemplate({ base: this.singleton.base(), repoId: _.get(br, 'repository.id'), branch: _.get(br, 'name') }), { headers: this.headers });
    ...
    loadData() {
        return __awaiter(this, void 0, void 0, function* () {
            const finalData = yield this.getBuilds();
            return finalData;
        });
    }
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const _ = __webpack_require__(5);
exports.repositoryURLTemplate = _.template('https://api.travis-ci.<%= base %>/owner/<%= owner %>/repos');
exports.branchesURLTemplate = _.template('https://api.travis-ci.<%= base %>/repo/<%= repoId %>/branches');
exports.buildsURLTemplate = _.template('https://api.travis-ci.<%= base %>/repo/<%= repoId %>/builds?branch.name=<%= branch %>&limit=10');
// POST methods cancel/restart build
exports.cancelBuildTemplate = _.template('https://api.travis-ci.<%= base %>/build/<%= buildId %>/cancel');
exports.restartBuildTemplate = _.template('https://api.travis-ci.<%= base %>/build/<%= buildId %>/restart');
...
const vscode_1 = __webpack_require__(1);
const _ = __webpack_require__(5);
const Git = __webpack_require__(11);
const ini = __webpack_require__(13);
const apiTemplates_1 = __webpack_require__(8);
const axios = __webpack_require__(14);
let instance;
const AccountType = {
...
class ActiveRepositorySingleton {
    constructor(context, _path) {
        this.context = context;
        this._path = _path;
        this._accountType = '';
    }
    // referenced from felixrieseberg/vsc-travis-ci-status
    static isTravisProject() {
        if (!vscode_1.workspace || !vscode_1.workspace.rootPath) {
            return false;
        }
        const conf = path.join(vscode_1.workspace.rootPath, '.travis.yml');
        try {
    ...
    static getAccountType() {
        return vscode_1.workspace.getConfiguration('travisClient').get('pro') === true ? 'enterprise' : 'community';
    }
    interval() {
    ...
    getRepositoryIdFromTravis() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const repo = _.head(_.filter(response.data.repositories, (repo) => repo.name === this.repository()));
    ...
    getProjectDetails(context = this.context) {
    ...
    setAuthToken(accountFlavour) {
        return __awaiter(this, void 0, void 0, function* () {
            let type = accountFlavour;
            if (!type) {
                type = this.accountType();
            }
            const accountContext = AccountType[type];
            const newToken = yield vscode_1.window.showInputBox({
            ...
            if (newToken) {
                ...
                vscode_1.commands.executeCommand('theTravisClient.refresh');
                vscode_1.window.showInformationMessage('You successfully added token, loading repositories wait a moment.');
            }
            else {
                vscode_1.window.showWarningMessage('You failed to add token, try again (Shift + CMD + P) travis set token');
            }
        });
    }
    setDetails(repoPath = this._path) {
        if (repoPath) {
            // time interval for auto refresh
            if (!this._timeInterval) {
                let interval = vscode_1.workspace.getConfiguration('travisClient').get('interval');
            ...
            if (!this._owner) {
                ...
                this._owner = vscode_1.workspace.getConfiguration('travisClient').get('owner');
            }
            ...
            if (!this._branch) {
                Git.branch(this._path, (err, activeBranch) => {
    ...
    repositoryId() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this._repositoryId) {
                yield this.getRepositoryIdFromTravis();
            }
            return this._repositoryId;
        });
    }
}
ActiveRepositorySingleton.createInstance = (context, repoPath) => {
    instance = new ActiveRepositorySingleton(context, repoPath);
};
ActiveRepositorySingleton.getInstance = () => {
    if (instance) {
        return instance;
    }
};
exports.ActiveRepositorySingleton = ActiveRepositorySingleton;
function setRepositoryDetails(repoPath) {
    if (path) {
        ...
        try {
            ...
            vscode_1.window.showErrorMessage('Make sure that git is configured properly.!');
            return ['', ''];
        }
    }
    else {
...
/* WEBPACK VAR INJECTION */(function(__dirname) {var exec = __webpack_require__(12).exec
...
var pkg = __webpack_require__(48);
...
const hasFlag = __webpack_require__(46);
...
class TravisStatusBar {
    constructor(context) {
        ...
        this._statusBarItem = vscode_1.window.createStatusBarItem(vscode_1.StatusBarAlignment.Left, 100);
        this.subscribe();
    ...
    loadStatusBarData() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const activeRepo = { id: yield this.singleton.repositoryId() };
            const response = yield axios.get(apiTemplates_1.buildsURLTemplate({
                base: this.singleton.base(),
                repoId: _.get(activeRepo, 'id'),
                branch: this.singleton.branch()
            }), { headers: headers });
            const build = _.head(_.get(response, 'data.builds'));
            const statusData = yield { id: build.id, repoName: this.singleton.repository(), branchName: this.singleton.branch(), state: build.state };
            return statusData;
        });
    }
    getStatusIcon(state) {
    ...
    updateStatusBar(refresh = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.singleton && this.singleton.isTravisProject()) {
                ...
                try {
                    const data = yield this.loadStatusBarData();
...
exports.TravisStatusBar = TravisStatusBar;
